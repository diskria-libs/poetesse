...
sealed interface KotlinCodeBlockTrait : KotlinCodeBlockFactory {
    ...
    operator fun List<KotlinCodeBlockCommand>.unaryPlus() {
        forEach {
            if (it.type == KotlinCodeBlockCommandType.STATEMENT) {
                +it.codeBlock
            } else {
                container.append(it.type, it.codeBlock)
            }
        }
    }
    ...
    operator fun KPCodeBlock.unaryPlus() {
        if (isEmpty()) return
        container.append(KotlinCodeBlockCommandType.STATEMENT, this)
    }
}
...
inline fun <reified T : Any> KotlinCodeBlockTrait.val_(
    noinline initializer: KotlinCodeScope.Block
...
inline fun <reified T : Any> KotlinCodeBlockTrait.var_(
    noinline initializer: KotlinCodeScope.Block
